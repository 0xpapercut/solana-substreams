...
impl AmmInstruction {
    ...
    pub fn unpack(input: &[u8]) -> Result<Self, &'static str> {
        let (&tag, rest) = input
            .split_first()
            .ok_or("Invalid instruction data")?;
        Ok(match tag {
            0 => {
            ...
            6 => {
                ...
                match AmmParams::from_u64(param as u64) {
                    AmmParams::AmmOwner => {
                        if rest.len() >= 32 {
                            let bytes: [u8; 32] = array_ref![rest, 0, 32].clone();
                            let new_pubkey = Some(Pubkey::from(bytes));
            ...
            15 => {
                ...
                match param {
                    0 | 1 => {
                        let bytes: [u8; 32] = array_ref![rest, 0, 32].clone();
                            let owner = Some(Pubkey::from(bytes));
            ...
            _ => return Err("Invalid instruction data"),
        })
    }

    fn unpack_u8(input: &[u8]) -> Result<(u8, &[u8]), &'static str> {
        if input.len() >= 1 {
            ...
            Err("Invalid instruction data")
        }
    }

    fn unpack_u16(input: &[u8]) -> Result<(u16, &[u8]), &'static str> {
        if input.len() >= 2 {
    ...
    pub fn pack(&self) -> Result<Vec<u8>, &'static str> {
                ...
                match AmmParams::from_u64(*param as u64) {
                    AmmParams::AmmOwner => {
                        let new_pubkey = match new_pubkey {
                            Some(a) => a,
                            None => return Err("Invalid instruction data"),
                        };
