...
pub fn raydium_events(block: Block) -> Result<pb::raydium::BlockRaydiumEvents, Error> {
    ...
    Ok(pb::raydium::BlockRaydiumEvents { transactions, slot })
}
...
fn parse_swap_instruction(
    ...
    let transfer_in = parse_token_transfer_instruction(
        &instruction.inner_instructions[instructions_len - 2],
        context,
    )?;
    let transfer_out = parse_token_transfer_instruction(
...
fn parse_initialize_instruction(
    ...
    let instructions_len = instruction.inner_instructions.len();
    let coin_transfer = parse_token_transfer_instruction(
        &instruction.inner_instructions[instructions_len - 3],
        context,
    )?;
    let pc_transfer = parse_token_transfer_instruction(
        &instruction.inner_instructions[instructions_len - 2],
        context,
    )?;
    let lp_mint_to = parse_token_mint_to_instruction(
        &instruction.inner_instructions[instructions_len - 1],
        context,
    )?;
...
fn parse_withdraw_instruction(
    ...
    let lp_mint_to = parse_token_burn_instruction(
        &instruction.inner_instructions[instructions_len - 1],
        context,
    )?;
...
fn parse_token_transfer_instruction(
    ...
    match spl_token_substream::parse_instruction(instruction, context) {
        Ok(Some(event)) => match event {
            spl_token_substream::pb::spl_token::spl_token_event::Event::Transfer(transfer) => {
                Ok(transfer)
            }
...
fn parse_token_mint_to_instruction(
    ...
    match spl_token_substream::parse_instruction(instruction, context) {
        Ok(Some(event)) => match event {
            spl_token_substream::pb::spl_token::spl_token_event::Event::MintTo(mint_to) => {
                Ok(mint_to)
            }
